...
final hookReentrantProvider =
    AsyncNotifierProvider.family<HookReentrantController, int, void Function()>(
...
final class HookReentrantController
    extends EventControllerNotifier<int, OuterEvent> {
...
void main() {
  test('reentrant records use one isolate-wide non-reentrant FIFO gate', () {
    ...
    innerContainer = ProviderContainer(
      overrides: [
        eventLoggerProvider.overrideWithValue(
          CallbackLogger((record) => recordDelivery('inner', record)),
        ),
      ],
    );
    final outerContainer = ProviderContainer(
      overrides: [
        eventLoggerProvider.overrideWithValue(
          CallbackLogger((record) => recordDelivery('outer', record)),
        ),
      ],
    );
    ...
    expect(delivered.take(2).map((entry) => entry.loggerIdentity), <String>[
      'outer',
      'inner',
    ]);
    expect(
      delivered.take(2).map((entry) => entry.record.phase),
      <EventLogPhase>[
    ...
    final sequences = delivered
        .take(2)
        .map((entry) => entry.record.recordSequence!)
        .toList();
    expect(
      sequences,
  ...
  test(
    'hook reentry delivers the reserved outer transition before later records',
    () async {
      late ProviderContainer innerContainer;
      var hookRan = false;
      final delivered = <({String loggerIdentity, EventLogRecord record})>[];
      final innerLogger = CallbackLogger(
        (record) => delivered.add((loggerIdentity: 'inner', record: record)),
      );
      final outerLogger = CallbackLogger(
        (record) => delivered.add((loggerIdentity: 'outer', record: record)),
      );
      innerContainer = ProviderContainer(
        overrides: [eventLoggerProvider.overrideWithValue(innerLogger)],
      );
      final outerContainer = ProviderContainer(
        overrides: [eventLoggerProvider.overrideWithValue(outerLogger)],
      );
      addTearDown(innerContainer.dispose);
      addTearDown(outerContainer.dispose);
      ...
      final outerTransitionIndex = delivered.indexWhere(
        (entry) =>
            entry.loggerIdentity == 'outer' &&
            entry.record.phase == EventLogPhase.transition,
      );
      final innerCreationIndex = delivered.indexWhere(
        (entry) =>
            entry.loggerIdentity == 'inner' &&
            entry.record.phase == EventLogPhase.controllerCreated,
      );
  ...
  test(
    'failed lazy logger resolution cannot block later framework records',
    () async {
      late ProviderContainer innerContainer;
      ...
      innerContainer = ProviderContainer(
        overrides: [
          eventLoggerProvider.overrideWithValue(
            CallbackLogger(innerRecords.add),
          ),
        ],
      );
      final outerContainer = ProviderContainer(
      ...
      await innerContainer
          .read(innerProvider.notifier)
          .dispatch(const IncrementInnerEvent());
  ...
  test('logger callbacks run without dispatch or trace context', () async {
    final observed =
        ...
        >[];
    final microtaskContext =
        Completer<
          ({EventDispatchContext? dispatchContext, TraceContext? traceContext})
        >();
    final logger = CallbackLogger((record) {
      observed.add((
        phase: record.phase,
      ...
      if (record.phase == EventLogPhase.transition &&
          !microtaskContext.isCompleted) {
        scheduleMicrotask(() {
